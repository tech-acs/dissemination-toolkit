<x-app-layout>
    ...
    <div class="flex flex-col max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <x-dissemination::message-display class="mt-6" />
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    @if ($record->name !== 'Super Admin')
                                        <a href="{{route('manage.role.edit', $record->id)}}" class="text-indigo-600 hover:text-indigo-900">{{ __('Edit') }}</a>
                                        <span class="text-gray-400 px-1">|</span>
                                        <a href="{{route('manage.role.destroy', $record->id)}}" x-on:click.prevent="confirmThenDelete($el)" class="text-red-600 hover:text-red-800">{{ __('Delete') }}</a>
                                    @endif
                                </td>
                            </tr>
                        @empty
