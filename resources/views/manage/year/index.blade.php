<x-app-layout>
    ...
    <div class="flex flex-col max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
        ...
        <div class="text-right">
            <a href="{{ route('manage.year.create') }}"><x-button>{{ __('Create new') }}</x-button></a>
        </div>
        @if (session('message'))
                            ...
                            @forelse($records as $record)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('manage.year.edit', $record->id) }}" class="text-indigo-600 hover:text-indigo-900">{{ __('Edit') }}</a>
                                        <span class="text-gray-400 px-1">|</span>
                                        <a href="{{ route('manage.year.destroy', $record->id) }}" x-on:click.prevent="confirmThenDelete($el)" class="text-red-600">{{ __('Delete') }}</a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
