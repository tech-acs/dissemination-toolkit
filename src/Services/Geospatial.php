...
class Geospatial
{
    public static function getGeoJsonByAreaId(array $areaIds)
    {
        $commaSeparatedIds = collect($areaIds)->join(',');
        $sql = "
            SELECT
                CASE
            ...
            (
                ...
                WHERE areas.id IN ($commaSeparatedIds) AND areas.geom IS NOT NULL
            ) AS filtered_areas
        ";
        try {
            //dump($sql);
