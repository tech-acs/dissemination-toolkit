        ...
        if ($this->isSortable) {
            if ($this->table->sortBy === $this->attribute) {
                if ($this->table->sortDirection === SortDirection::ASC) {
                    return Blade::render('<x-dissemination::icon.sort-asc class="text-blue-600" />');
                } else {
                    return Blade::render('<x-dissemination::icon.sort-desc class="text-blue-600" />');
                }
            }
            return Blade::render('<x-dissemination::icon.sort-asc />');
        }
        return '';
    }
